    ...
    execute: async (message, args, client, prefix) => {
        ...
        try {
            ...
            if (args.length) {
                ...
                let lowvolumebut = new MessageButton().setCustomId(`lowvolume_but_${message.guildId}`).setEmoji("<:voldown:994130330246201374>").setStyle("SECONDARY").setDisabled(disabled);

                let highvolumebut = new MessageButton().setCustomId(`highvolume_but_${message.guildId}`).setEmoji("<:volup:994130357173624923>").setStyle("SECONDARY").setDisabled(disabled);

                let previousbut = new MessageButton().setCustomId(`previous_but_${message.guildId}`).setEmoji("<:m_previous:994130242757210172>").setStyle("SECONDARY").setDisabled(disabled);

                let skipbut = new MessageButton().setCustomId(`skipbut_but_${message.guildId}`).setEmoji("<:m_next:994130188789104670>").setStyle("SECONDARY").setDisabled(disabled);
